...
const FeedbackPage = () => {
  ...
  const submissionHandler = async (e) => {
    ...
    const res = await fetch(
      "https://form-validation-2d58b-default-rtdb.firebaseio.com/contact.json",
      {
        method: "POST",
  ...
  const formModalContent = (
    <React.Fragment>
      <h1 className="mb-4 text-center text-paleBlue text-sm md:text-xl lg:text-xl font-semibold">
        REACH OUT TO US
      </h1>
      ...
      <div className="flex flex-col mb-3">
        ...
        {!formValidity.name && <p className="text-error-red">Please enter a valid name!</p>}
      </div>

      <div className="flex flex-col mb-3">
        <label htmlFor="email">Email</label>
        <input type="email" className={inputClasses} ref={emailInputRef} />
        {!formValidity.email && <p className="text-error-red">Please enter a valid email address!</p>}
      </div>
      ...
      <div className="flex flex-col mb-3">
        ...
        <textarea
          ...
          ref={messageInputRef}
        />
        {!formValidity.message && <p className="text-error-red">Please enter a valid message!</p>}
      </div>

      <button
        className="w-[100%] bg-paleBlue text-center text-xl text-White py-2 rounded-[5px] hover:bg-Marine-blue cursor-pointer"
  ...
  return (
    <Card className="w-[80%] md:w-[50%] lg:w-[30%] m-auto mt-24 p-4 flex flex-col">
